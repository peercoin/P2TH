...
Applications that model their transactions as a chain of transactions can use such a tree to map it's transactions on.
Therefore Finding all transactions for a single tag, becomes considerably more expensive than for a single tag address, as it requires an extra address lookup for every transaction.
However, since every transaction has a unique tag address, attacking the entire application becomes practically infeasible.
