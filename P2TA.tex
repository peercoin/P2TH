...
Contrary to the Peershares~\cite{Lee13} project that creates it's own blockchain, the PSA (Peercoin simple assets) paper proposes a way to register, distribute and trade assets on the Peercoin~\cite{King12} blockchain.
...
\section{Conclusion}
Tagging transactions allows thin clients for blockchain applications that rely on historical data.
This is accomplished by using only standard lookup functions for addresses and tags.
The tag address creation based on generating the private key from a publicly known salt results in insecure addresses.
...
An alternative could be to generate the public key or address based on the salt.
Or an address could be generated from a secret tag while only communicating the tag address itself.
By not sending funds to an unspendable address, the funds will be redeemed by anyone knowing or guessing the tag and salt, leaving the blockchain some extra puzzles.
